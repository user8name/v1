...
$wp_products_tasks = NULL;

//
// Table class for wp_products_tasks
//
class cwp_products_tasks extends cTable {
	var $id;
	var $title;
	var $filename;
	var $filepath;
	...
	function __construct() {
		...
		$this->TableVar = 'wp_products_tasks';
		$this->TableName = 'wp_products_tasks';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`wp_products_tasks`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('wp_products_tasks', 'wp_products_tasks', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// title
		$this->title = new cField('wp_products_tasks', 'wp_products_tasks', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// filename
		$this->filename = new cField('wp_products_tasks', 'wp_products_tasks', 'x_filename', 'filename', '`filename`', '`filename`', 200, -1, FALSE, '`filename`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->fields['filename'] = &$this->filename;

		// filepath
		$this->filepath = new cField('wp_products_tasks', 'wp_products_tasks', 'x_filepath', 'filepath', '`filepath`', '`filepath`', 200, -1, TRUE, '`filepath`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->filepath->UploadAllowedFileExt = "xls,xlsx";
		$this->fields['filepath'] = &$this->filepath;

		// status
		$this->status = new cField('wp_products_tasks', 'wp_products_tasks', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->fields['status'] = &$this->status;

		// userid
		$this->_userid = new cField('wp_products_tasks', 'wp_products_tasks', 'x__userid', 'userid', '`userid`', '`userid`', 3, -1, FALSE, '`userid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->_userid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userid'] = &$this->_userid;

		// username
		$this->username = new cField('wp_products_tasks', 'wp_products_tasks', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->fields['username'] = &$this->username;

		// itime
		$this->itime = new cField('wp_products_tasks', 'wp_products_tasks', 'x_itime', 'itime', '`itime`', 'DATE_FORMAT(`itime`, \'%y-%m-%d %H:%i:%s\')', 135, -1, FALSE, '`itime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['itime'] = &$this->itime;

		// utime
		$this->utime = new cField('wp_products_tasks', 'wp_products_tasks', 'x_utime', 'utime', '`utime`', 'DATE_FORMAT(`utime`, \'%y-%m-%d %H:%i:%s\')', 135, -1, FALSE, '`utime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['utime'] = &$this->utime;

		// remark
		$this->remark = new cField('wp_products_tasks', 'wp_products_tasks', 'x_remark', 'remark', '`remark`', '`remark`', 201, -1, FALSE, '`remark`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['remark'] = &$this->remark;

		// msg
		$this->msg = new cField('wp_products_tasks', 'wp_products_tasks', 'x_msg', 'msg', '`msg`', '`msg`', 201, -1, FALSE, '`msg`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`wp_products_tasks`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "wp_products_taskslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("wp_products_tasksview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("wp_products_tasksview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "wp_products_tasksadd.php?" . $this->UrlParm($parm);
		else
			$url = "wp_products_tasksadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("wp_products_tasksedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("wp_products_tasksadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("wp_products_tasksdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$ext = pathinfo($rsnew['filepath'], PATHINFO_EXTENSION);
		$rsnew['filename'] = $rsnew['filepath'];
		$rsnew['filepath'] = md5($rsnew['filepath'].intval(time())).".".$ext;
		...
		if ( 0 == $current_user->ID ) {  
			...
			// Logged in.
			$rsnew['userid'] = $current_user->ID;
			$rsnew['username'] = $current_user->user_email;
		}  
		return TRUE;
	}
