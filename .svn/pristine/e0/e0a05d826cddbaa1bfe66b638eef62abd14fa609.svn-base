<?php
/**
 * Plugin Name: Email Subscribers & Newsletters For Matexcel
	...
	if( $options['action'] == 'update' && $options['type'] == 'plugin' && isset( $options['plugins'] ) ) {
		...
		foreach( $options['plugins'] as $plugin ) {
			...
			if( $plugin == $our_plugin ) {
				$es_plugin_meta_data = get_plugin_data( WP_PLUGIN_DIR.'/email-subscribers-plus/email-subscribers-plus.php' );
