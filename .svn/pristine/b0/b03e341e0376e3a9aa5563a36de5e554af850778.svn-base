...
final class GoogleSitemapGenerator
{
    ...
    private function LoadOptions()
    {
        ...
        if ($storedOptions && is_array($storedOptions)) {
            ...
            update_option("sm_options", $this->options);
        }
    ...
    public function GetXmlUrl($type = "", $params = "", $buildOptions = array())
    {
        ...
        if (!empty($baseUrlSettings)) {
            $baseURL = $baseUrlSettings;
        } else if (defined("SM_BASE_URL") && SM_BASE_URL) {
            $baseURL = SM_BASE_URL;
        }
    ...
    public function DoRobots()
    {
        ...
        if (preg_match('/matexcel\.com/i', $host)) {
    ...
    public function AddUrl($loc, $lastMod = 0, $changeFreq = "monthly", $priority = 0.5, $postID = 0)
    {
        ...
        if ($this->GetOption('in_lastmod') === false) {
            $lastMod = 0;
        }
    ...
    private function SendStats()
    {
        ...
        if ($postCount <= 5) {
            ...
            $postCount = 25;
        } else if ($postCount < 75) {
            $postCount = 50;
        } else if ($postCount < 125) {
            $postCount = 100;
        } else if ($postCount < 2000) {
            $postCount = round($postCount / 200) * 200;
        } else if ($postCount < 10000) {
            $postCount = round($postCount / 1000) * 1000;
        } else {
            $postCount = round($postCount / 10000) * 10000;
        }
