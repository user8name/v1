...
class GoogleSitemapGeneratorStandardBuilder {
	...
	public function BuildProducts($gsg) {
		/** @var $wpdb wpdb */
		global $wpdb;
		...
		$rows = $wpdb->get_results($wpdb->prepare('SELECT `id`,`cid`,`cat`,`productname`,`description`,`seo_url`,`particlesize`,`mw`,`utime` FROM `wp_products` where isdel=%d',0));

		if($rows && is_array($rows)) {
			foreach($rows as $obj) {
				$url = $obj->seo_url;
				if ($url == '') {
					$url = home_url( '/' ).'p/' . $obj->id . '/'. sanitize_title(preg_replace( '/[^A-Za-z0-9\s\-]/', '-',$obj->productname)) . '/';
				}else{
					$url = home_url( '/' ).$url;
				}
				
				$gsg->AddUrl($url, $gsg->GetTimestampFromMySql($obj->utime), $gsg->GetOption("cf_posts"), $gsg->GetOption("pr_posts"));
			}
		}
	...
	public function Index($gsg) {
		...
		if($hasEnabledPostTypesPosts) $gsg->AddSitemap("products", null, $blogUpdate);
