...
$wp_products_add = NULL; // Initialize page object first

class cwp_products_add extends cwp_products {
	...
	// Page object name
	var $PageObjName = 'wp_products_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "wp_productsview.php")
	...
	function LoadDefaultValues() {
		...
		$this->particlesize->CurrentValue = NULL;
		$this->particlesize->OldValue = $this->particlesize->CurrentValue;
		$this->mw->CurrentValue = NULL;
		$this->mw->OldValue = $this->mw->CurrentValue;
		...
		$this->sortnum->CurrentValue = 0;
		$this->status->CurrentValue = 0;
		$this->isdel->CurrentValue = 0;
		$this->seo_title->CurrentValue = NULL;
		$this->seo_title->OldValue = $this->seo_title->CurrentValue;
		$this->seo_keywords->CurrentValue = NULL;
		$this->seo_keywords->OldValue = $this->seo_keywords->CurrentValue;
		$this->seo_description->CurrentValue = NULL;
		$this->seo_description->OldValue = $this->seo_description->CurrentValue;
		$this->seo_url->CurrentValue = NULL;
		$this->seo_url->OldValue = $this->seo_url->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->cat->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(cat = '" . ew_AdjustSql($this->cat->CurrentValue, $this->DBID) . "')";
		...
		// cid
		$this->cid->SetDbValueDef($rsnew, $this->cid->CurrentValue, 0, strval($this->cid->CurrentValue) == "");
		...
		// particlesize
		$this->particlesize->SetDbValueDef($rsnew, $this->particlesize->CurrentValue, NULL, FALSE);

		// mw
		$this->mw->SetDbValueDef($rsnew, $this->mw->CurrentValue, NULL, FALSE);
		...
		// sortnum
		$this->sortnum->SetDbValueDef($rsnew, $this->sortnum->CurrentValue, 0, strval($this->sortnum->CurrentValue) == "");

		// status
		$this->status->SetDbValueDef($rsnew, $this->status->CurrentValue, 0, strval($this->status->CurrentValue) == "");

		// isdel
		$this->isdel->SetDbValueDef($rsnew, $this->isdel->CurrentValue, 0, strval($this->isdel->CurrentValue) == "");

		// seo_title
		$this->seo_title->SetDbValueDef($rsnew, $this->seo_title->CurrentValue, NULL, FALSE);

		// seo_keywords
		$this->seo_keywords->SetDbValueDef($rsnew, $this->seo_keywords->CurrentValue, NULL, FALSE);

		// seo_description
		$this->seo_description->SetDbValueDef($rsnew, $this->seo_description->CurrentValue, NULL, FALSE);

		// seo_url
		$this->seo_url->SetDbValueDef($rsnew, $this->seo_url->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($wp_products_add)) $wp_products_add = new cwp_products_add();

// Page init
$wp_products_add->Page_Init();

// Page main
$wp_products_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$wp_products_add->Page_Render();
?>
...
var CurrentForm = fwp_productsadd = new ew_Form("fwp_productsadd", "add");

// Validate form
fwp_productsadd.Validate = function() {
...
fwp_productsadd.Form_CustomValidate = 
...
fwp_productsadd.ValidateRequired = true;
<?php } else { ?>
fwp_productsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fwp_productsadd.Lists["x_cid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_fullname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $wp_products_add->ShowPageHeader(); ?>
<?php
$wp_products_add->ShowMessage();
?>
<form name="fwp_productsadd" id="fwp_productsadd" class="<?php echo $wp_products_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($wp_products_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $wp_products_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $wp_products_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fwp_productsadd.Init();
</script>
<?php
$wp_products_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$wp_products_add->Page_Terminate();
?>
